...
def draw_boxes(frame, tracked_objects):
    ...
    for track in tracked_objects:
        if len(track) == 7:  # Expected format: [x1, y1, x2, y2, track_id, conf, cls_id]
            x1, y1, x2, y2, track_id, conf, cls_id = map(int, track)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='CCTV Human Tracking using YOLO and DeepSORT')
    ...
    parser.add_argument('--reid_model', type=str, default=REID_MODEL, 
